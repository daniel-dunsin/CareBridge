...
let AppointmentProvider = class AppointmentProvider {
  ...
  async validatePatientAndDocAvailaibility(
    sessionDto,
    doctorId,
    patientId,
    isPatient,
    ...
    const data = await this.appointmentService.getAppointments({
      ...
      status: {
        $nin: [
          ...
          enums_1.AppointmentStatus.FAILED,
        ],
      },
    });
    return {
  ...
  async getAppointment(appointmentId) {
    ...
    if (!data) throw new common_1.NotFoundException('Appointment not found');
    return {
      success: true,
  ...
  async rescheduleAppointment(sessionDto, appointmentId, user) {
    ...
    if (
      ...
      throw new common_1.BadRequestException(
        'Only the doctor/patient of this appointment can reschedule it',
      );
    }
    await this.validatePatientAndDocAvailaibility(
      sessionDto,
      ...
      user.role === enums_2.RoleNames.PATIENT,
    );
    ...
    const emailContext = {
      ...
      prevAppointmentDate: (0, date_fns_1.format)(
      ...
      newAppointmentDate: (0, date_fns_1.format)(
        sessionDto.appointmentDate,
        'do, MMM yyyy',
      ),
  ...
  async cancelAppointment(appointmentId, user) {
    ...
    await this.mailService.sendMail({
      ...
      context: {
        ...
        appointmentDate: (0, date_fns_1.format)(
          appointment.appointmentDate,
          'do, MMM yyyy',
        ),
    ...
    if (appointments.length > 0) {
      await Promise.allSettled(
        appointments.map(async (appointment) => {
          ...
          const startTime = (0, date_fns_1.format)(
