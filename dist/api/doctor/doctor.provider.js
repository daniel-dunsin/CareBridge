...
let DoctorProvider = class DoctorProvider {
    ...
    async updateDoctor(userId, updateDoctorDto) {
        const data = await this.doctorService.updateDoctor({ user: new mongoose_1.Types.ObjectId(userId) }, updateDoctorDto);
        if (!data)
            throw new common_1.NotFoundException('Doctor not found');
    ...
    async updateKycDocuments(updateKycDto, userId) {
        ...
        const { url: idDoc, public_id: idDocPublicId } = await this.fileService.uploadResource(updateKycDto.idDoc);
        const { url: professionalCert, public_id: professionalCertPublicId } = await this.fileService.uploadResource(updateKycDto.professionalCert);
