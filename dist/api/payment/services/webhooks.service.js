...
let WebhookService = class WebhookService {
  ...
  validateWebhook(signature, webhookResponse) {
    ...
    if (hash != signature)
      throw new common_1.ForbiddenException(
        'oops! this endpoint is only authorized for paystack',
      );
  }
  ...
  async successfulMedicinePurchase(chargeResponse) {
    const attempt = await this.paymentService.updatePaymentAttempt(
      ...
      { status: enums_1.PaymentStatus.SUCCESSFUL },
    );
    if (!attempt) throw new common_1.NotFoundException('transaction not found');
