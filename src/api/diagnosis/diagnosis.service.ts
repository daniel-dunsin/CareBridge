...
export class DiagnosisService {
  constructor(
    @InjectModel(BoneMetrics.name)
    private readonly _boneMetricsModel: Model<BoneMetricsDocument>,
    @InjectModel(BrainMetrics.name)
    private readonly _brainMetricsModel: Model<BrainMetricsDocument>,
    @InjectModel(EyesMetrics.name)
    private readonly _eyesMetricsModel: Model<EyesMetricsDocument>,
    @InjectModel(HeartMetrics.name)
    private readonly _heartMetricsModel: Model<HeartMetricsDocument>,
    @InjectModel(KidneyMetrics.name)
    private readonly _kidneyMetricsModel: Model<KidneyMetricsDocument>,
    @InjectModel(LiverMetrics.name)
    private readonly _liverMetricsModel: Model<LiverMetricsDocument>,
    @InjectModel(SkinMetrics.name)
    private readonly _skinMetricsModel: Model<SkinMetricsDocument>,
    @InjectModel(TeethMetrics.name)
    private readonly _teethMetricsModel: Model<TeethMetricsDocument>,
  ) {}
  ...
  async mapDepartmentToModel(department: Departments) {
    ...
    if (!model)
      throw new InternalServerErrorException(
        'Unable to store records for the selected department',
      );
  ...
  async getSingleDiagnosis<T extends Document>(
