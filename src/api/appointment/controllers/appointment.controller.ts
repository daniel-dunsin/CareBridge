...
export class AppointmentController {
  ...
  async bookSession(
    ...
    const data = await this.appointmentProvider.bookSession(
      bookSessionDto,
      doctorId,
      user,
    );
  ...
  async getUserDoctorAppointments(@Auth('_id') userId: string) {
    const data =
      await this.appointmentProvider.getUserDoctorAppointments(userId);
  ...
  async getPatientAppointments(
    ...
    const data =
      await this.appointmentProvider.getPatientAppointments(patientId);
  ...
  async getUserPatientAppointments(@Auth('_id') userId: string) {
    const data =
      await this.appointmentProvider.getUserPatientAppointments(userId);
  ...
  async getUserPendingAppointments(@Auth() user: UserDocument) {
    const data =
      await this.appointmentProvider.getUserPendingAppointments(user);
    return data;
  }
  ...
  async rescheduleAppointment(
    ...
    const data = await this.appointmentProvider.rescheduleAppointment(
      sessionDto,
      appointmentId,
      user,
    );
  ...
  async cancelAppointment(
    ...
    const data = await this.appointmentProvider.cancelAppointment(
      appointmentId,
      user,
    );
  ...
  async generateMeetingLink(
    ...
    const data = await this.appointmentProvider.createMeetingLink(
      appointmentId,
      join_url,
    );
