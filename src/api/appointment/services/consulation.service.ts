...
export class ConsultationService {
  constructor(
    ...
    private readonly _consultationModel: Model<ConsultationDocument>,
  ) {
  ...
  async createConsultation<T>(
    createConsultaionDto: T,
  ...
  async getConsultation(
    ...
    const consulation = await this.populate(
      this._consultationModel.findOne(filter),
    );
  ...
  async getConsultations(
    ...
    const consulation = await this.populate(
      this._consultationModel.find(filter).sort({ createdAt: -1 }),
    );
  ...
  async deleteConsultation(
    ...
    const consulation = await this._consultationModel.findOneAndDelete(
      filter,
      options,
    );
